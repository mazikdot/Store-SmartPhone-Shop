...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_POST['update'])) {
        ...
        $sql = "update phone_status set status_name=:status_name  where status_id=:status_id";
        $query = $dbh->prepare($sql);
        $query->bindParam(':status_name', $status_name, PDO::PARAM_STR);
                                        ...
                                        <div class="wizard-content">
                                            <div class="row">
                                                <div class="col m6">
                                                    <div class="row">
                                                        <?php
                                                        $eid = intval($_GET['status_id']);
                                                        $sql = "SELECT * from  phone_status WHERE status_id=:status_id";
                                                        $query = $dbh->prepare($sql);
                                                        $query->bindParam(':status_id', $eid, PDO::PARAM_STR);
                                                        $query->execute();
                                                        ...
                                                        if ($query->rowCount() > 0) {
                                                            foreach ($results as $result) {               ?>
                                                                <div class="input-field col  s12">
                                                                    <label for="status_name">แก้ไขรุ่นโทรศัพท์มือถือ</label>
                                                                    <input name="status_name" id="status_name" value="<?php echo htmlentities($result->status_name); ?>" type="text" autocomplete="off" required>
